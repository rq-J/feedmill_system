...
@section('content')
    <div class="bg-white shadow-lg rounded container">
        <h3 class="pt-4">
            ...
            </a>&nbsp;Pivot Logs - Downtime</h3>
        <div class="row">
            <div class="col-12">
                @livewire('add-downtime')
            </div>
            <hr>
            <table id="downtime" class="table table-bordered table-hover text-nowrap" width="100%">
                <thead>
                <tr>
                    <th scope="col">No</th>
...
@section('scripts')
  <script>
    $(document).ready(function () {
      let downtime = $('#downtime').DataTable({
        processing: true,
        serverSide: true,
        scrollX: true,
    ...
    $(document).on('click', '#refresh', function(e) {
      var downtime = $('#downtime').DataTable();
      downtime.ajax.reload();
    });
    ...
    $(document).on('click', '#update', function (e) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          var update_url = "{{ route('pivot.dt.update') }}"
    ...
    $(document).on('click', '#remove', function (e) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          var update_url = "{{ route('pivot.dt.remove') }}"
          window.location.replace(update_url + "/" + id);
        }
        else {
          Swal.fire({
